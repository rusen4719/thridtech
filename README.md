# Rocketalk
전남대학교 전기·전자통신·컴퓨터공학부(여수) 졸업작품<br>
작성자 : 이재룡 (프론트 엔드 담당)
...
Rest Api가 중요하다는건 알고 있었지만 아직도 어떤 개념인지 확실하지는 모르겠습니다. 그리고 Retrofit을 사용해서 서버와 HTTP 통신 하는데
GET 방식인지 POST인지 주고받는 데이터의 타입이 서버측과 클라이언트측이 동일해야 통신이 되는 부분에서 애를 먹었습니다.<br> 
이부분을 이해하려고 서버측 코드까지 공부하게 되었습니다. 하지만 이로 인해 node.js 코드는 어느정도 볼 수 있게 됬네요!
